<template>
  <div class="adminMonthlyPaymentNewPlan flex-column">
    <YfDetailsHeader title="月度入账计划" :path="'/expendManagement/adminMonthlyPayment'+this.detailsInfo.hash"></YfDetailsHeader>
    <div class="monInNewContent">
      <YfSearch tActive="month" @confirm="submit" :plantId="detailsInfo.plantId" :time="detailsInfo.dataDate" timeTypeName="计划付款月份：" :checkDis="true" layout="plant,dateTypeRange"></YfSearch>
      <el-table
        :data="MIncomeData"
        border
        style="width: 100%">
        ...
        <div class="SContent">
          <ul>
            <li v-for="(item, index) in bookedMaterial" :key="index">
              <span>
                <span class="color1 bgColor1">{{item.fileCode===800?'收据':'项目验收单'}}</span>
              </span>
...
export default {
  ...
  methods: {
    init () {
      ...
      this.$http.fetchGet(`/plan/getPayInfo?planId=${this.detailsInfo.id}`).then(res => {
    ...
    submitAudit () {
      this.$http.fetch(`/plan/plantPlanAudit`, {
        ...
        if (res.code === 0) {
          this.$router.push('/expendManagement/adminMonthlyPayment')
        }
      })
    },
    download (row) {
