...
<template>
  <div class="supervision flex-column">
    <YfDetailsHeader :path="formRoute" title="指派检修人" print @handlePrint="handlePrint"></YfDetailsHeader>
    ...
    <div class="flex-1 content flex-column">
      ...
      <div class="flex-1 scrollWrap" style="overflow: auto;" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0)">
        <YfCollapse title="基础信息"  style="padding: 5px 15px 0 7px;" ref="collapse1" key="1" v-if="activeName=='#/step_1'">
          <baseForm ref="baseForm" :prosessCode="1" :superviseType="superviseType" :procCategoryId="procCategoryId" :formDataOld="baseFormData" @resize="resize"></baseForm>
        </YfCollapse>
        ...
        <YfCollapse title="故障信息"  style="padding: 5px 15px 0 7px;" ref="collapse4" key="4" v-if="activeName=='#/step_2'&&superviseType==1">
          <errorInfoTable :prosessCode="4"></errorInfoTable>
        </YfCollapse>
        <YfCollapse title="故障处理"  style="padding: 5px 15px 0 7px;" ref="collapse4" key="7" v-if="activeName=='#/step_2'&&superviseType==1">
          <errorDealwith :prosessCode="4"></errorDealwith>
        </YfCollapse>
      ...
      <div class="contentBottom flex-center">
        <a class="rightBtn flex-center" @click="designate(false)">
          <span>取消</span>
        </a>
        <a class="rightBtn primary flex-center" @click="designate(true)">
          <span>确定派单</span>
        </a>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 确定指派
    designate: function(issure){
      if(issure){
        this.$refs.baseForm.$refs.form1.validate((valid) => {
          if (valid) {
            let _formData = JSON.parse(JSON.stringify(this.$refs.baseForm.formData))
            let url
            if(_formData.procCategoryId ==42){
              url = `supvAssign/updateSupvAssignApproval`
            }else{
              url = `supvAssign/updateSupvAssign`
            }
            let data = {
              "createAccount": this.userInfo.userAccount,
              "maintainerId": _formData.maintainerId,
              "superviseId": _formData.superviseId
            }
            this.loading = true
            this.$http.fetch(url,data).then(res=>{
              this.loading = false
              if(res.code==0&&!this._isDestroyed){
                this.$message.success(_formData.procCategoryId ==42?'已批准!':'派单成功!')
                this.$router.push(this.formRoute)
              }
            })
          } else {
            return false;
