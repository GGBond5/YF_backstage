<template>
  <div class="monthlyPaymentNewRecord flex-column">
    <YfDetailsHeader title="月度付款计划" :path="'/expendManagement/monthlyPayment'+this.monDetails.hash"></YfDetailsHeader>
    <div class="monInNewContent">
      <YfSearch tActive="month" @confirm="submit" :plantId="monDetails.plantId" :time="monDetails.dataDate" timeTypeName="计划付款月份：" :checkDis="true"  layout="plant,dateTypeRange"></YfSearch>
      <el-table
        :data="MIncomeData"
        border
        style="width: 100%">
        ...
        <el-table-column
          prop="yearAmount"
          :label="`${monDetails.dataDate?$moment(monDetails.dataDate).format('yyyy'):'-'}年付款计划金额`">
          <template slot-scope="scope">
        ...
        <div class="SContent">
          <ul>
            <li v-for="(item, index) in bookedMaterial" :key="index">
              <span>
                <span class="color1 bgColor1">{{item.fileCode === 800?'收据':'项目付款'}}</span>
              </span>
...
export default {
  ...
  methods: {
    init () {
      ...
      this.$http.fetchGet(`/plan/getPayInfo?planId=${this.monDetails.id}`).then(res => {
        this.MIncomeData = res.data.months
        this.MIncomeData.map((val, index) => {
          val.serialNumber = index + 1
          val.operation = parseInt(val.monthAmount) > parseInt(val.actualAmount?val.actualAmount:'0')? '添加付款记录': '查看'
        })
      })
    },
    addMonthRecord () {
