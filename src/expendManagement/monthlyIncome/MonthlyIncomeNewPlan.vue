<template>
  <div class="monthlyIncomeNewPlan flex-column">
    <YfDetailsHeader title="月度入账计划" path="/expendManagement/monthlyIncome"></YfDetailsHeader>
    <div class="monInNewContent flex-1 flex-column">
      <YfSearch tActive="month" @plantSelect="monPlantSelect" @selectMonth="monSelectMonth" timeTypeName="计划入账月份：" @confirm="submit" layout="plant,dateTypeRange"></YfSearch>
      <el-table
        :data="MIncomeData"
        border
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div :class="setBlue(scope.row)" @click="newProject(scope.row)"><i class="el-icon-plus" v-if="scope.row.serialNum === '新增月度入账项目'" style="marginRight: 5px"></i>{{scope.row.serialNum}}</div>
          </template>
        </el-table-column>
        <el-table-column
          prop="tenderName"
        ...
        <el-table-column
          prop="planAmount"
          :label="`${this.monthTime?$moment(this.monthTime).format('yyyy'):'-'}年入账计划金额`">
          <template slot-scope="scope">
            <span v-format="'#,##0.####'">{{scope.row.planAmount}}</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="monthAmountSum"
          ...
          <template slot-scope="scope">
            <div class="flex-row" style="textAlign: left;">
              <el-upload
                ...
                <el-button slot="trigger" class="uploadBtn"><i class="el-icon-plus"></i>发票</el-button>
              </el-upload>
...
export default {
  ...
  methods: {
    ...
    setBlue (row) {
      if (['新增月度入账项目'].includes(row.serialNum)) { //新增入账计划的样式
        return {
          blueTextBtn: true
        }
      }
    ...
    newProject () {
      ...
      this.$http.fetchGet(`/project/getBillProjectList?date=${this.monthTime}&plantId=${this.plant}`).then(res => {
    ...
    pitchOn () {
      this.MIncomeData = [{serialNum: '新增月度入账项目'}]
    ...
    MEnter (e, materials) {
      ...
      this.$nextTick(() => {
        ...
        this.$refs.uploadList.style.top = `${DOMRect.top - 100 - DOMRect.height}px`
      })
    },
    ...
    cellClick (row, column, cell) {
      if (['本月计划入账金额', '备注'].includes(column.label)) {
    ...
    submitAudit () {
      ...
      this.$http.fetch(`/plan/billPlanEdit`, {
        ...
        if (res.code === 0) {
          ...
          this.$router.push('/expendManagement/monthlyIncome')
        }
      })
    },
    temporaryStorage () {
